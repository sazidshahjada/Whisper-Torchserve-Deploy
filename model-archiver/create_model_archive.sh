...
if [ ! -d "/home/model-server/handler" ]; then
...
ls -la /home/model-server/handler/

# Check if the main handler file exists
if [ ! -f "/home/model-server/handler/whisper_handler.py" ]; then
...
torch-model-archiver --model-name whisper \
  --version 1.0 \
  --handler /home/model-server/handler/whisper_handler.py \
  --runtime python \
